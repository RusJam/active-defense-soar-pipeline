...
![n8n Automation Architecture Flow](n8n-workflow.png)

1. **Threat Simulation:** Network enumeration and brute-force authentication cycles are executed against a Windows target host.
2. **Telemetry Collection:** The Windows Event Viewer captures the authentication failures and passes the raw logs to a centralized Wazuh SIEM manager.
3. **Automated Containment:** The SIEM engine recognizes the attack signature and immediately deploys an active response script on the target host. This script uses the native Windows firewall to drop all incoming traffic from the attacking IP address for a specified cooldown window.
4. **Orchestration and Routing:** Simultaneously, the SIEM manager dispatches a structured webhook payload to an n8n automation engine.
5. **Local AI Enrichment:** The automation engine passes the alert metadata to a local Ollama instance running Llama 3 to generate a targeted security summary without exposing sensitive infrastructure data to the public cloud.
6. **Case Generation:** The enriched data is written directly to a newly initialized incident case inside TheHive 5 dashboard.
7. **ChatOps Notification:** The final triaged output is pushed to a dedicated security operations communication channel.
...
![Wazuh Active Response Firewall Isolation](wazuh-alert.png)
...
#### Enterprise Incident Case Creation
![TheHive 5 Security Ticket Logging](thehive-ticket.png)

#### Live Analyst Broadcast
![Discord SecOps ChatOps Notification](discord-alert.png)
